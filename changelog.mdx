---
title: "Changelog: every published CLI version"
sidebarTitle: "Changelog"
description: "Every Retasc CLI release includes the date it was published to npm. The release notes identify the issue behind every change."
"og:image": "https://docs.retasc.com/images/og/changelog.png?v=e6c6a1f1"
"og:image:alt": "retasc docs: Changelog: every published CLI version"
"twitter:image": "https://docs.retasc.com/images/og/changelog.png?v=e6c6a1f1"
"twitter:image:alt": "retasc docs: Changelog: every published CLI version"
---
...
date, and each entry names the issue behind it. One line per change; the full notes sit
under each version's fold.
...
<div className="rt-when">2026-09-19</div>

- **RTSC-1057** — the proxy now reports how long **this machine** keeps a session transcript, read from the harness's own settings (Claude Code's `cleanupPeriodDays`), so the Dash can name the day a conversation will be gone instead of quoting a documented default at you.

<Accordion title="Full notes">
...
- **RTSC-1061** — nothing asks which subscription pays for an agent any more, so `retasc plan` is gone and `retasc bind` no longer puts the question.
...
<div className="rt-when">2026-09-18</div>

- **RTSC-1042** — the proxy now sweeps this machine's dead worktrees on the first claim of a session.
...
<div className="rt-when">2026-09-15</div>

- **RTSC-1014** — every tool you start in a bound folder now gets its **own credential**, so one can be retired without stopping the others.
...
<div className="rt-when">2026-09-14</div>

- **RTSC-958** — a user never names an agent, so `--agent <name>` is gone from `init`, `bind`, `key mint` and `join`.
...
<div className="rt-when">2026-09-13</div>

- **RTSC-962** — the proxy now reports git worktrees nobody claimed, so Retasc can see work an agent started without saying so.
...
<div className="rt-when">2026-09-11</div>

- **RTSC-901** — `retasc plan`: which subscription pays for each of your agents, listed and set one line per (agent, harness).
- **RTSC-901** — `bind` asks the same question at setup, but only when `--runtime` was STATED.
...
<div className="rt-when">2026-09-09</div>

- **RTSC-879** — the proxy sends NO `Authorization` header when it has no key, instead of `Bearer ` with nothing after it.
...
- **RTSC-879** — a committed `.mcp.json` now names `npx -y @retasc/cli@<version> mcp-proxy` instead of a bare `retasc`, so it starts on a machine that is not the one that wrote it.
- **RTSC-879** — `retasc bind --json` can now sign in from a container.
- **RTSC-879** — `retasc doctor` no longer installs a package to check a marker.
- **RTSC-879** — the repo's own committed `.mcp.json`, and the marker blocks published in `web/auth.md` and the agent-skill, now show the npx form.
- **[no-issue]** — test fix: the setup-hook test pinned `RETASC_HOME` to a temp dir.
...
<div className="rt-when">2026-09-08</div>

- **RTSC-878** — `retasc doctor` no longer condemns the entry `retasc setup` writes.
...
- **RTSC-864** — `retasc key mint` now prints every remote-key config block a hosted agent can paste, including the TOML one that did not exist.
...
- **RTSC-855** — `retasc doctor` and `retasc setup` now say whether the **Retasc Agent Skill** is installed on this machine, and print the one command that installs it.
...
- **RTSC-861** — the proxy now tells Retasc which CLI version it is.
...
<div className="rt-when">2026-09-05</div>

- **RTSC-799** - `tidy`, `done` and `claim` now work out which branch is this repo's trunk instead of assuming it is called `main`.
...
- **RTSC-832** — the model now actually appears.
...
- **RTSC-821** — a session now reports WHICH MODEL ran it.
...
<div className="rt-when">2026-09-04</div>

- **RTSC-812** — no runtime change.
...
- **RTSC-825** — every workspace now carries the name of the folder it is bound to.
...
- **RTSC-820** — every session now tells Retasc which conversation it is.
...
<div className="rt-when">2026-09-03</div>

- **RTSC-810** — `retasc unbind` revokes the key it says it revokes.
- **RTSC-810** — `retasc bind --org-id X --project-id Y` learns the project's prefix.
...
<div className="rt-when">2026-09-02</div>

- **RTSC-801** — (security) `save_attachment_file` no longer reads a workspace's own secrets, and no longer follows a file swapped in after it was checked.
...
- **RTSC-808** — `retasc setup` no longer writes OpenCode's config into your real home when `RETASC_HOME` says otherwise.
...
- **RTSC-800** — (security) a keystore-resolved key now travels only to the keystore's own url, whether the competing url arrives in the `.mcp.json` entry or in the environment.
...
<div className="rt-when">2026-08-30</div>

- **RTSC-793** — `retasc setup` now wires Cursor, OpenCode and Gemini CLI too, bringing it to six harnesses.
...
<div className="rt-when">2026-08-28</div>

- **RTSC-780 follow-up** — `retasc init` now sets a folder up the same way every other door does.
...
- **RTSC-780** — `retasc setup` wires Retasc into every MCP harness on the machine, once, and `retasc bind` now runs it for you.
- **RTSC-781** — `--runtime` now says what it does.
...
<div className="rt-when">2026-08-26</div>

- **RTSC-749** — `retasc triage` reads and approves work filed from outside your org.
...
<div className="rt-when">2026-08-25</div>

- **RTSC-645** — `retasc gate install` no longer throws away your edits.
...
- **RTSC-520** — the CLI now says when it is out of date.
...
- **RTSC-743** — the last CLI replies that answered a person with a machine shape now answer in sentences.
...
- **RTSC-478** — a network failure now names its own cause.
...
- **RTSC-709** — `retasc issue show` and `retasc issue list` read the queue from a terminal.
- **RTSC-709** — `retasc checkpoint` and `retasc check-claim` close the claim lifecycle.
- **RTSC-709** — `done` reads the issue back before it closes it.
- **RTSC-709** — `cli/README.md` now records which MCP tools are deliberately Dash-and-MCP-only, and why, so their absence from the CLI reads as a decision rather than an oversight.
...
- **RTSC-741** — a rejected write no longer costs you the lease.
...
<div className="rt-when">2026-08-24</div>
...
<div className="rt-when">2026-08-23</div>
...
- **RTSC-720** — when `bind --json` stops to ask, it hands your agent the exact questions as data: what to call the workspace, the first project, its prefix, and a confirmation naming the precise folder about to be connected — with the flag each answer fills.
- **RTSC-721** — `retasc unbind` exists.
- **RTSC-721** — `bind` heals a dead binding instead of interrogating you about it.
...
- **RTSC-713** — your agent can set Retasc up for you.
- **RTSC-713** — `bind --json` reports what it did and what it still needs, one JSON object per line, and prints the approve URL the moment it exists rather than at the end.
- **RTSC-713** — a setup that paused is no longer dressed as a crash.
- **RTSC-713** — sign-in no longer refuses merely because nothing is attached to a terminal.
- **RTSC-713** — every outcome names the absolute folder it is about to connect, so a wrong one can be caught before anything is written.
- **RTSC-691** — `retasc login` opens your browser instead of asking you to retype an eight-character code.
- **RTSC-691** — the code path stays for the places a browser round trip cannot work: inside a container, and over SSH, where the redirect would target `127.0.0.1` on whichever machine opened the link and so could never reach the CLI waiting on the remote host.
- **RTSC-691** — `retasc release <RTSC-NN> --claim-token <token>` exists.
- **RTSC-691** — `retasc bind` prints "restart your agent" whether or not a human is watching.
- **RTSC-691** — `bind` clears the credential-less bootstrap entry that would otherwise shadow the binding it just wrote, when the two land in different scopes.
...
<div className="rt-when">2026-08-20</div>

- **RTSC-681** — your agent can read a file you attached to an issue.
- **RTSC-681** — downloads land under `.retasc/attachments/` in your workspace, one folder per attachment, never overwriting anything and never following a symlink, in a directory that ignores itself so a customer's certificate cannot be committed by accident.
...
<div className="rt-when">2026-08-19</div>

- **RTSC-672** — every command we hand out now says `npx @retasc/cli@latest`.
- **RTSC-672** — a first-run failure names the build that produced it, so "it still doesn't work" and "you're running last week's CLI" stop looking identical.
...
- **RTSC-676** — signing in opens your browser.
- **RTSC-676** — the finish screen's closing lines are flush left, and it ends with a blank line instead of welding the shell prompt to the last thing you read.
...
- **RTSC-673** — `bind` ends by telling you what to do, not by dumping config.
...
- **RTSC-670** — `retasc bind` can sign you in again.
- **RTSC-671** — errors name the command you actually ran.
...
- **RTSC-669** — the invite project picker takes several projects.
...
- **RTSC-667** — `retasc members invite` asks instead of demanding ids.
- **RTSC-666** — the PROJECTS column in `retasc members list` comma-joins, so two projects read as `XTRO, XTRO Marketing` rather than running together.
- **RTSC-664** — invite someone to one project, not the whole org.
...
<div className="rt-when">2026-08-17</div>

- **RTSC-660** — the proxy attaches files for you.
...
<div className="rt-when">2026-08-12</div>

- **RTSC-646** — the watchdog now says when it is **not** renewing a lease.
...
<div className="rt-when">2026-08-10</div>

- **RTSC-643** — `retasc gate install` keys the gate to **this folder's** project, not the machine-wide default.
...
<div className="rt-when">2026-08-07</div>

- **RTSC-527** — the re-import warning dates the last import in **your** timezone.
...
<div className="rt-when">2026-08-04</div>
...
<div className="rt-when">2026-08-03</div>

- **RTSC-495** — `retasc bind --setup <code>` sets a folder up with no sign-in and nothing to answer, so an agent can do it on behalf of someone who does not use a terminal.
...
<div className="rt-when">2026-08-02</div>

- **RTSC-530** — setting up from scratch now asks where your work comes from, and imports it in the same command.
...
- **RTSC-524** — new command: `retasc import`.
...
- **RTSC-522** — the changelog is public, and five releases that were never written down now are.
...
- **RTSC-507** — "none of these are me" now covers the tools you were asked about, not the whole org forever.
...
<div className="rt-when">2026-08-01</div>

- **RTSC-477** — new command: `retasc identity`.
...
<div className="rt-when">2026-07-31</div>

- **RTSC-492** — `retasc join` is now the whole of an invited teammate's setup, in one command run from the folder their agent will work in.
- **RTSC-498** — `retasc doctor` now says which platform the CLI is actually tested on.
...
- **RTSC-493** — the MCP marker no longer names a command that may not exist.
...
<div className="rt-when">2026-07-21</div>

- **RTSC-297** — `retasc bind` no longer strands a new org silently.
...
- **RTSC-321** — the liveness watchdog now stops heartbeating an issue the moment it moves to `review` (RTSC-257): the server releases the lease there, so the proxy no longer pings a dead lease until the next heartbeat fails.
...
- **RTSC-306** — `retasc claim` / `retasc next` now pull **lane-scoped by default**: only issues assigned to your principal or left unassigned, never another human's work.
...
<div className="rt-when">2026-07-20</div>

- **RTSC-266** — README now links the setup page, the docs quickstart, and the parallel-agents guide instead of only the bare homepage.
...
- **RTSC-269** — `retasc bind`'s org/project picker no longer treats a typo as "create new".
- **RTSC-269** — the picker gives up after 3 invalid answers instead of looping, and a closed stdin (Ctrl-D, or a pipe running dry) now exits with an error rather than hanging.
...
- **RTSC-263** — owner/member permission denials now print a readable reason and the next step instead of an opaque `Server Error`.
- **RTSC-263** — `formatError` now strips U+0085, U+2028 and U+2029 in addition to the ASCII control characters.
...
- **RTSC-262** — `doctor` and `whoami` no longer report "not bound" for a folder whose Retasc MCP is registered in **Claude Code local scope** (the `claude mcp add` default — stored in Claude's own config, not `./.mcp.json`).
- **RTSC-262** — `claim`/`tidy`/`done` resolve the key through the same entry selection, so the CLI can no longer say "✓ bound" in one command and "No Retasc MCP key found" in another for the same folder.
- **RTSC-262** — the illegal-global-server check is now **presence-based**: a hand-added user-scope entry is detected in every shape (including keyless), and an unreadable `~/.claude.json` reports "cannot verify" instead of a false all-clear.
- **RTSC-262** — honest failure modes: server-unreachable (timeouts, 5xx, rate limits) is no longer reported as a rejected key, `whoami`/`doctor` requests are bounded by a 10s timeout, and server-derived strings are stripped of control characters before printing.
- **RTSC-262** — `bind` hardening: re-binding to the *same* org/project is an idempotent success (no key churn for provisioning scripts); replacing a *different* binding non-interactively without `--yes` now fails loudly with exit 1 instead of silently succeeding as a no-op; a cloned repo's committed marker no longer triggers the replace prompt (minting your own key under it is what `bind` is for).
- **RTSC-262** — (security) a keystore-resolved key now only ever travels to the keystore's own URL — a hostile `RETASC_MCP_URL` planted next to a committed secret-free marker can no longer redirect a teammate's real key.
...
- **RTSC-261** — backend failures you're meant to act on now print what actually went wrong instead of `✗ Server Error`.
...
<div className="rt-when">2026-07-19</div>

- **RTSC-279** — new `retasc billing` command: the org's whole billing picture in the terminal instead of only in the Dash.
- **RTSC-279** — (server-side, no CLI change) the MCP usage meter now reports **two** clearly-named figures instead of one misleading `wouldBeBillUsd`: `lifetimeMeteredEstimateUsd` (all-time activity repriced at today's rate card — a diagnostic, not an amount owed) and `pendingUsd` (what is actually owed right now, the same figure the Dash shows).
...
<div className="rt-when">2026-07-17</div>

- **RTSC-250** — serialize `patchConfig` under a cross-process advisory lock so concurrent CLI/MCP processes can't lose an update.
...
- **RTSC-179** — atomic config write (temp file + rename) so a reader never sees a truncated `config.json`; adds `RETASC_DIR` support.
- **RTSC-178** — token refresh now classifies transient backend errors vs a genuinely expired refresh token, so a one-off backend blip no longer drags a valid session through a full device re-login.

## 1.3.0

<div className="rt-when">2026-07-08</div>

- Auto-reap branches on MCP close: when the watchdog proxy observes a session's own claim close terminally and successfully, it removes that issue's clean, merged `rtsc-NN/*` worktree and branch out-of-band (`retasc tidy --prune --only <id>`), instead of orphaning them until a human ran `retasc tidy`.
...
<div className="rt-when">2026-07-07</div>

- **RTSC-170** — credential-file permission hardening: `config.json`, `bindings.json`, and the fallback `.mcp.json` are written `0600` (dirs `0700`) with no world-readable creation window.

## 1.2.3

<div className="rt-when">2026-07-05</div>

- **RTSC-150** — watchdog `isClaimLost` fix: no longer drops a live lease when a payload field merely contains the string `CLAIM_LOST`.

## 1.2.2

<div className="rt-when">2026-07-04</div>

- **RTSC-148** — `retasc claim` takes the issue as a positional argument, and never silently falls through to `next_issue`.
...
- **RTSC-143** — one tolerant tool-result parser, and a loud failure when the claim fence is off.
...
- **RTSC-137** — org-membership commands: `retasc members invite|list|revoke` and `retasc join`.

## 1.1.3

<div className="rt-when">2026-07-02</div>
...
- **RTSC-130** — republish the bind auto-wire fix that had changed CLI source without a version bump (npm versions are immutable).

## 1.1.1

<div className="rt-when">2026-06-28</div>

- **RTSC-98** — `claim`, `tidy` and `done` resolve the keystore key through the same resolver the proxy uses.
...
- **RTSC-92** — workspace keys move to a home keystore, and the folder marker becomes secret-free.
...
<div className="rt-when">2026-06-27</div>
