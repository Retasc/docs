...
Retasc is an MCP server, not another web app. Describe a problem and your agent files
it as an issue; Claude Code, Codex and Cursor agents then call `next_issue` and each
claims one unblocked, prioritized task. The swarm runs in parallel: no collisions, no
human dispatching.
