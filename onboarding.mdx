...
<Note>
  **A lane scopes dispatch, it doesn't lock the work.** `next_issue` and `next_batch` hand
  out issues assigned to *you* plus everything unassigned, so your agents never drain a
  teammate's assigned work. When a pull comes back empty it reports `otherLanes`, the count
  of ready issues sitting in someone else's lane. Pass `allLanes: true` to pull from every
  lane, or take one outright with `claim_issue RTSC-42`, which ignores lanes entirely. A
  claim is refused only for a reason the server states: the issue is a tracking container
  (`work:false`), a live lease is already held on it, its status is out of play (done,
  canceled, or blocked), an open blocker is in the way, you're the one who submitted it for
  review, or you're over your claim or spending cap.
</Note>
